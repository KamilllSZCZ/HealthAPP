...
const COLORS = ["#10D9A6", "#3B82F6", "#22C55E", "#F59E0B", "#EC4899", "#22D3EE"];
...
export default function Habits() {
  ...
  const toggle = async (h: any) => {
    ...
    try {
      ...
      toast.error(errMessage(e, "Couldn't update habit."));
    }
  };
  ...
  const create = async () => {
    ...
    try {
      ...
      toast.error(errMessage(e, "Couldn't create habit."));
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (h: any) => {
    const ok = await confirmAsync("Delete habit?", `"${h.name}" and its streak history will be permanently removed.`);
    if (!ok) return;
    try {
      ...
      toast.error(errMessage(e, "Couldn't delete habit."));
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        {habits.length === 0 ? (
          <EmptyState icon="repeat-outline" title="No habits yet" subtitle="Create your first habit to start building streaks." />
        ) : (
          <>
            ...
            {habits.map((h) => (
              <Card key={h.id} style={styles.item} testID={`habit-${h.id}`}>
                ...
                {!h.system && (
                  <TouchableOpacity onPress={() => remove(h)} style={styles.delBtn} testID={`delete-habit-${h.id}`}>
